...
test.describe('Feature Flag Service API Tests (v3)', () => {
  ...
  test('GET ListAvailable - should return 200 with list of flags', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlags();
  ...
  test('GET ListAvailable - should validate response schema', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlags();
    expect(response.status()).toBe(200);
    const body = await response.json();
  ...
  test('GET ListAvailable - response should contain flag name strings', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlags();
    const body = await response.json();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
  ...
  test('GET IsAvailable - should return 200 with true for a known flag', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.isFlagAvailable('FeatureTest1');
  ...
  test('GET IsAvailable - should validate response schema', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.isFlagAvailable('FeatureTest1');
    expect(response.status()).toBe(200);
    const body = await response.text();
    await test.info().attach('Schema Validation', {
  ...
  test('GET IsAvailable - should handle non-existent flag name', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.isFlagAvailable('NonExistentFlag_12345');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET IsAvailable - should handle empty flag name', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.isFlagAvailable('');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET IsAvailable - should handle special characters in flag name', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.isFlagAvailable('Flag!@#$%^&*()');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET IsAvailable - should handle very long flag name', async ({ featureFlagClientV3 }) => {
    ...
    const response = await featureFlagClientV3.isFlagAvailable(longName);
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET ListAvailable by category - should return 200 for valid category', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('QA');
    const body = await response.json();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET ListAvailable by category - should validate response schema', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('QA');
    expect(response.status()).toBe(200);
    const body = await response.json();
    await test.info().attach('Schema Validation', {
  ...
  test('GET ListAvailable by category - should handle non-existent category', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('NonExistentCategory_99999');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET ListAvailable by category - should handle empty category', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET ListAvailable by category - should handle special characters in category', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('Cat!@#$%');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
      contentType: 'text/plain',
  ...
  test('GET ListAvailable by category - should handle case sensitivity', async ({ featureFlagClientV3 }) => {
    const response = await featureFlagClientV3.listAvailableFlagsByCategory('qa');
    const body = await response.text();
    await test.info().attach('API Response', {
      body: JSON.stringify({ status: response.status(), body }, null, 2),
  ...
  test('E2E - list all flags, check specific flag, list by category', async ({ featureFlagClientV3 }) => {
    // Step 1: List all available flags
    const listResponse = await featureFlagClientV3.listAvailableFlags();
    ...
    const categoryResponse = await featureFlagClientV3.listAvailableFlagsByCategory('QA');
